...
def test_workers_registry_flow_start_workers_and_check_status():
    ...
    queues_registry.load([
        { 'name': 'records', 'type': 'eventsflow.queues.local.EventsQueue' }
    ])
    ...
    assert [ type(w) for w in workers_registry.workers(status='active')]    == [ SampleProcessingWorker, ] 

    queues_registry.get('records').publish(EventStopProcessing())
    time.sleep(1)

    assert [ type(w) for w in workers_registry.workers(status='active')]    == [ ] 
    assert [ type(w) for w in workers_registry.workers(status='inactive')]    == [ SampleProcessingWorker, ] 
